...
const ItemFormPickTime = ({ item, availableTimes, setAvailableTimes }) => {
    ...
    const [timeRange3, setTimeRange3] = useState([])
    const [currentTimeQuadrant, setCurrentTimeQuadrant] = useState('')
    const timeDate = new Date()
    const timeString = timeDate.toLocaleTimeString()

    useEffect(() => {
        if (timeString.slice(3, 5) > '0' && timeString.slice(3, 5) <= '15') setCurrentTimeQuadrant(1)
        if (timeString.slice(3, 5) > '15' && timeString.slice(3, 5) <= '30') setCurrentTimeQuadrant(2)
        if (timeString.slice(3, 5) > '30' && timeString.slice(3, 5) <= '45') setCurrentTimeQuadrant(3)
        if (timeString.slice(3, 5) > '45') setCurrentTimeQuadrant(4)
    })
    ...
    useEffect(() => {
        setTimeRange1(timeSelection.slice(Number(timeString.slice(0, 2)) * 4 + currentTimeQuadrant))
        setTimeRange2(timeSelection)
        setTimeRange3(timeSelection.slice(0, Number(timeString.slice(0, 2)) * 4 + currentTimeQuadrant + 1))
    ...
    const displayTimes = (day) => {
        let date
        let timeRange
        if (day === 'day1') { date = day1 }
        if (day === 'day1') { timeRange = timeRange1 }
        if (day === 'day2') { date = day2 }
        if (day === 'day2') { timeRange = timeRange2 }
        if (day === 'day3') { date = day3 }
        if (day === 'day3') { timeRange = timeRange3 }
        return (
            <>
                <p style={{ marginTop: "50px" }}>Showing Times for {date}</p>
                <div className={FormCSS.timeGrid}>
                    ...
                    {(timeString.slice(3, 5) > '15' && timeString.slice(3, 5) <= '30' && day === "day1") && <><div className={FormCSS.timeBoxOuter}></div><div className={FormCSS.timeBoxOuter}></div></>}
